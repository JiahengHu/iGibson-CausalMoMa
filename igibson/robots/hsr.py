...
BODY_LINEAR_VELOCITY = 0.08 # 0.05  # linear velocity thresholds in meters/frame
BODY_ANGULAR_VELOCITY = 0.08 #0.05  # angular velocity thresholds in radians/frame
BODY_MOVING_FORCE = 500
EPSILON = 0.001
WHEEL_FRICTION = 0.001

HEAD_VELOCITY = 1.0
ARM_VELOCITY = np.array([0.2, 1.2, 2.0, 1.5, 1.5])
...
class HSR(ManipulationRobot, loco_robot_class, ActiveCameraRobot):
    ...
    def _default_base_controller_configs(self):
        dic = {
            ...
            "command_output_limits": (
                np.array([-BODY_LINEAR_VELOCITY] * 2 + [-BODY_ANGULAR_VELOCITY] ),
                np.array([BODY_LINEAR_VELOCITY] * 2 + [BODY_ANGULAR_VELOCITY] ),
            ),
        }
        return dic

    # Switching to position control for all HSR
    @property
    ...
    def _default_camera_joint_controller_config(self):
        ...
        return {
            ...
            "command_output_limits": (
                np.array([-HEAD_VELOCITY / joint_vel_scale] * 2),
                np.array([HEAD_VELOCITY / joint_vel_scale] * 2),
                ),
            "use_delta_commands": True,
        }
    ...
    def _default_controller_config(self):
        ...
        cfg["arm_{}".format(self.default_arm)]["InverseKinematicsController"]["ik_joint_idx"] = np.array(
            [
                self.joint_idx_to_ik_joint_idx[x]
                for x in cfg["arm_{}".format(self.default_arm)]["InverseKinematicsController"]["joint_idx"]
            ]
        )
    ...
    def hsr_joint_position(self):
        ...
        alj = 0.22
        default[self.jn2i["arm_lift_joint"]] = alj
        default[self.jn2i["torso_lift_joint"]] = alj / 2
